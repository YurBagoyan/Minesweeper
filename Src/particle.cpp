...
void particle_draw(particle *p, size_t size) {
    ...
    for (i = 0; i < size; ++i) {
        ...
        float line = (p[i].pos[0] - p[i].center[0]) * (p[i].pos[0] - p[i].center[0]);
        float colsc = ((p[i].pos[1] - p[i].center[1]) * 0.6) * ((p[i].pos[1] - p[i].center[1]) * 0.6); 
        
        float distance = sqrt(line + colsc);
